    ...
    try:
        ...
        for segment in arr:
            ...
            if segment != "":
                ...
                value = 0
                if re.search("[0-9]*(d|D)[0-9F]+", segment): #if there's a dice (regex FTW!)
        ...
        phenny.reply("You are a bad person for trying to make me roll that.")
...
#Time for the real fun, dice!
    if(diceroll.startswith('d')): #check if it's XdX or dX
        ...
        if diceroll.split('d')[1] == 'f':
            return rollfudge(rolls)
...
def rollfudge(n = 4):
    res = []
    for i in range(n):
        res.append(str(choice([randint(-1,1) for x in range(10)])))
    return "("+"+".join(res)+")"
