...
def question(phenny, input): 
    ...
    if word:
        ...
        tmp = word.split('|')
        if len(tmp) != 1:
            word = ', or '.join(['({0}) {1}'.format(i+1,x) for i, x in enumerate(tmp)])
...
def factoid(phenny, input):
    """A database of facts"""
    if not (input.sender.startswith('#') or phenny.admin):
        phenny.say('You are not authorized to use factoids this way.')
        return
    ...
    if no:
        ...
        if not oldfactoid:
            ...
            return
        newfactoid = "|".join((oldfactoid, factoid))
