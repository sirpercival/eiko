...
def manage_rss(phenny, input):
    """.rss operation channel site_name url -- 'add', 'del', or 'list' rss"""
    if not input.admin:
        phenny.reply("Sorry, you need to be an admin to modify the RSS feeds.")
        return
    ...
    if len(text) < 2:
        phenny.reply("Proper usage: '.rss add ##channel Site_Name URL', '.rss del ##channel Site_Name URL', '.rss del ##channel'")
    elif len(text) > 2:
    ...
    if len(text) > 4 and text[1] == 'add':
        ...
        if len(temp) == 1:
            ...
            phenny.reply("Not enough parameters specified.")
            return
        if fg_colour:
        ...
        conn.commit()
        c.close()
        phenny.reply("Successfully added values to database.")
    elif len(text) == 3 and text[1] == 'del':
        # .rss del ##channel
        c.execute("DELETE FROM rss WHERE channel = ?", (channel,))
        conn.commit()
        c.close()
        phenny.reply("Successfully removed values from database.")
        ...
        conn.commit()
        c.close()
        phenny.reply("Successfully removed the site from the given channel.")
    elif len(text) == 2 and text[1] == 'list':
        c.execute("SELECT * FROM rss")
        k = 0
        for row in c:
            k += 1
            phenny.say("list: " + unicode(row))
        if k == 0:
            phenny.reply("No entries in database")
    else:
        phenny.reply("Incorrect parameters specified.")
    c.close()
...
def read_feeds(phenny):
    ...
    if not c.fetchall():
        STOP = True
        phenny.say("No RSS feeds found in database. Please add some rss feeds.")
    ...
    for row in c:
        ...
        try:
            ...
            phenny.say("Can't parse.")

        try:
            entry = fp.entries[0]
        except:
            if DEBUG:
                phenny.say("row: " + str(row))
                phenny.say("Can't find element: " + str(fp))
            continue
        ...
        if len(cursor_recent.fetchall()) < 1:
            ...
            phenny.msg(feed_channel, response)
            ...
            if DEBUG:
                if hasattr(phenny.config, 'logchan_pm'):
                    phenny.msg(phenny.config.logchan_pm, u"Skipping previously read entry for %s: %s %s" % (feed_channel, site_name_effect, entry.title))
                else:
                    phenny.say(u"Skipping previously read entry for %s: %s %s" % (feed_channel, site_name_effect, entry.title))
    cursor_recent.close()
    c.close()
...
def startrss(phenny, input):
    ...
    if not input.admin:
        phenny.reply("You must be an admin to start up the RSS feeds.")
        return
    ...
    if query == '-v':
        DEBUG = True
        STOP = False
        phenny.reply("Debugging enabled.")
    elif query == '-q':
        DEBUG = False
        STOP = False
        phenny.reply("Debugging disabled.")
    elif query == '-i':
        INTERVAL = input.group(3)
        phenny.reply("INTERVAL updated to: %s" % (str(INTERVAL)))
    elif query == '--stop':
        STOP = True
        phenny.reply("Stop parameter updated.")
